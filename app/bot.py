    ...
    async def cmd_predict(message: types.Message):
        ...
        try:
            ...
            text = (
                ...
                f"📈 *Вероятности:*\n"
                f"   • П1: {result['decision']['home_prob']}%\n"
                f"   • X:  {result['decision']['draw_prob']}%\n"
                f"   • П2: {result['decision']['away_prob']}%\n"
                f"🎯 *Прогноз:* {result['decision']['winner_name']} ({result['decision']['winner_probability']}%)\n"
                f"🧮 *Ожидаемый счёт:* {result['decision']['expected_score']}\n"
                f"🔒 *Уверенность:* {result['decision']['confidence']}%\n"
                f"📌 *Топ-5 счетов:*\n"
            )
            for score, prob in result.get("top_scores", []):
