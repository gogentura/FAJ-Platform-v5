...
class FAJXGModel:
    ...
    def calculate(
        ...
        ВАЖНО:
        home_rating / away_rating пока логируются и возвращаются
        в components, но напрямую не вмешиваются в xG.

        Это сделано специально, чтобы не смешивать
        FAJ Rating и Power Profile до отдельной калибровки.
        """
    ...
    def _form_factor(
        ...
        """
        Преобразование формы 0-100
        в коэффициент влияния.
        ...
        Неверное/отсутствующее значение:
        1.00
        """
    ...
    def _control_factors(
        ...
        Раньше один control_factor умножал xG
        ОБЕИХ команд.
    ...
    def _build_explanation(
        ...
        if home_control_factor > 1.05:
            explanation.append(
                f"Контроль даёт преимущество хозяев "
                f"({home_control_factor:.2f}x)"
            )

        elif away_control_factor > 1.05:
            explanation.append(
                f"Контроль даёт преимущество гостей "
                f"({away_control_factor:.2f}x)"
            )
        ...
        if home["form"] > 1.05:
            explanation.append(
                f"Форма хозяев повышает атакующий потенциал "
                f"({home['form']:.2f}x)"
            )

        elif home["form"] < 0.95:
            explanation.append(
                f"Форма хозяев снижает атакующий потенциал "
                f"({home['form']:.2f}x)"
            )

        if away["form"] > 1.05:
            explanation.append(
                f"Форма гостей повышает атакующий потенциал "
                f"({away['form']:.2f}x)"
            )

        elif away["form"] < 0.95:
            explanation.append(
                f"Форма гостей снижает атакующий потенциал "
                f"({away['form']:.2f}x)"
            )
