...
class PredictionManager:
    ...
    def predict(
        ...
        ВНИМАНИЕ: Основной способ вызова — через predict_by_match_id().
        """
    ...
    def _get_current_season_id(self) -> Optional[int]:
        """Ищет сезон 2026-2027. ИСПРАВЛЕНО: убран fallback."""
        ...
        try:
            ...
            if row:
                season_id = row[0]
                logger.info("Season detected: %s", season_id)
                return season_id
    ...
    def _get_match(self, match_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            cursor.execute(
                """
                SELECT
                    m.id, m.round_id, m.home_team_id, m.away_team_id,
                    m.date, m.competition, m.status, m.actual_home, m.actual_away,
    ...
    def _save_prediction(
        ...
        try:
            ...
            logger.info("Prediction saved | prediction_id=%s | match_id=%s", pred_id, match_id)
            return pred_id
    ...
    def _find_match_by_teams(self, home_team: str, away_team: str) -> Optional[int]:
        """Находит матч в SQLite. ТОЛЬКО как fallback."""
        conn = self.db.get_connection()
