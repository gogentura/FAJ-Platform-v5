...
Историческое окно передаётся в хронологическом порядке:

    M1 → M2 → M3 → M4 → M5 → M6

где:
    M1 = самый старый матч окна
    M6 = самый свежий матч перед прогнозируемой игрой

Порядок является частью Brain Contract.
Ни один downstream-модуль не должен
самостоятельно переворачивать историю.

FormModel использует этот порядок для:
    - temporal weights (M6 получает максимальный вес)
    - OLS trend (временная последовательность)

    M6 = последний матч перед прогнозом → максимальный вес.
    M1 = самый старый матч окна → минимальный вес.
"""
