...
Архитектура:
    - Счёт → ручной ввод (пользователь)
    - Статистика → NB-BET (парсер)
...
def extract_prediction_score(prediction: Dict[str, Any]) -> Optional[str]:
    ...
    for key in ("predicted_home_goals", "expected_home_goals", "faj_home_goals", "home_xg"):
        if prediction.get(key) is not None:
            home = safe_float(prediction.get(key))
            break

    for key in ("predicted_away_goals", "expected_away_goals", "faj_away_goals", "away_xg"):
        if prediction.get(key) is not None:
...
def load_stats_from_url(url: str) -> Dict[str, Any]:
    """
    Загружает статистику через NbBetStatsParser.
    Возвращает только stats.
    """
    if not url or not url.strip():
        return {"stats": {}, "success": False, "data_quality": 0.0}

    parser = NbBetStatsParser()
    result = parser.parse_stats(url.strip())
    ...
    return {
        ...
        "source": result.get("source", "nb-bet"),
    }
                ...
                try:
                    ...
                    if result["success"] and result["stats"]:
                        ...
                        fact = {
                            ...
                            "parser_source": result.get("source", "nb-bet"),
                            "parser_version": result.get("parser_version", PARSER_VERSION),
                            "data_quality": result.get("data_quality", 0.0),
                        }
                        st.session_state[f"{key_prefix}_fact"] = fact
                        st.success(f"✅ Статистика загружена ({len(result['stats'])} показателей)")
                        st.rerun()
                    else:
                        st.warning("⚠️ Статистика не найдена. Проверьте ссылку.")

                except Exception as exc:
                    logger.exception("Ошибка загрузки статистики")
