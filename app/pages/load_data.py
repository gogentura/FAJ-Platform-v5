...
with st.sidebar:
    ...
    # ============================================================
    if st.button("📅 Загрузить календарь", use_container_width=True):
        st.session_state.page = 'load_calendar'
    
    if st.button("📊 Загрузить статистику", use_container_width=True):
...
# ----- ПРОГНОЗЫ -----
    ...
    if teams_df.empty:
        ...
        if st.button("📅 Перейти к загрузке календаря"):
            st.session_state.page = 'load_calendar'
            st.rerun()
    else:
        col1, col2 = st.columns(2)
