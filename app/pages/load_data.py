...
import sqlite3
import os
from app.parsers.championat_parser import ChampionatParser
...
def main():
    ...
    - ✅ Все матчи РПЛ 2026/27 (30 туров, 240 матчей)
    - ✅ Результаты уже сыгранных матчей
    - ✅ Обновит паспорта команд
    """)
    ...
    if st.button("🔥 ЗАГРУЗИТЬ ДАННЫЕ", type="primary", use_container_width=True):
        ...
        try:
            ...
            # Команды РПЛ
            from app.parsers.championat_parser import RPL_TEAMS
            ...
            with st.spinner("Парсим календарь с championat.com..."):
                parser = ChampionatParser()
                matches = parser.parse()
            
            if not matches:
                st.error("❌ Не удалось загрузить календарь. Попробуйте позже.")
                return
            ...
            for match in matches:
                ...
                date = match.get('match_date', '2026-08-14')
                time = match.get('match_time', '19:00')
                ...
                # Если матч завершён, добавляем результаты
                if status == 'finished':
