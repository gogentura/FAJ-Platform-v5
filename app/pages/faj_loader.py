...
# ============================================================
# 24 МАТЧА (ДАННЫЕ ИЗ ТВОИХ ССЫЛОК)
# ============================================================
...
def save_statistics(cursor, match_id, team_id, possession, shots, shots_on_target, corners, yellow_cards, xg, pass_accuracy):
    ...
    columns = ["match_id", "team_id", "possession", "shots", "shots_on_target", "corners", "yellow_cards", "xg", "pass_accuracy"]
    values = [match_id, team_id, possession, shots, shots_on_target, corners, yellow_cards, xg, pass_accuracy]
    valid_columns = []
    valid_values = []
    ...
    if valid_columns:
        ...
        cursor.execute(f"INSERT INTO match_statistics ({column_sql}) VALUES ({placeholders})", valid_values)

def main():
    st.set_page_config(page_title="FAJ Loader", layout="wide")
    st.title("📥 FAJ Loader")
    st.subheader("РПЛ 2026/27 — загрузка 24 фактических матчей")
    st.info("Будут загружены фактические результаты и статистика 1–3 туров РПЛ. Всего 24 матча.")
    st.metric("Матчей к загрузке", len(MATCH_DATA))

    if st.button("🔥 ЗАГРУЗИТЬ ВСЕ 24 МАТЧА", type="primary", use_container_width=True):
        conn = None
        try:
            conn = get_connection()
            cursor = conn.cursor()
            ...
            # ============================================================
            # АВТОМАТИЧЕСКОЕ ОПРЕДЕЛЕНИЕ СТРУКТУРЫ seasons
            # ============================================================
            ...
            for index, row in enumerate(MATCH_DATA):
                ...
                # Статистика
                save_statistics(cursor, match_id, home_id, home_possession, home_shots, home_sot,
                               home_corners, home_yellow, home_xg, home_pass_accuracy)
                save_statistics(cursor, match_id, away_id, away_possession, away_shots, away_sot,
                               away_corners, away_yellow, away_xg, away_pass_accuracy)
                ...
                cursor.execute("""
                    UPDATE matches SET
                        ...
                        status = 'finished'
                    WHERE id = ?
                """, (home_goals, away_goals, home_xg, away_xg, home_possession, away_possession,
                      home_shots, away_shots, home_sot, away_sot, match_id))
            ...
            with col1:
                st.metric("Завершённых матчей", finished_count)
            with col2:
                st.metric("Результатов", results_count)
            with col3:
                st.metric("Статистических записей", statistics_count)

            st.divider()
            st.subheader("📋 Загруженные 24 матча")

            cursor.execute("""
                SELECT r.round_number, ht.name, at.name, m.actual_home, m.actual_away, m.home_xg, m.away_xg
                FROM matches m
                JOIN rounds r ON r.id = m.round_id
                JOIN teams ht ON ht.id = m.home_team_id
                JOIN teams at ON at.id = m.away_team_id
                WHERE r.season_id = ? AND r.round_number IN (1, 2, 3) AND m.status = 'finished'
