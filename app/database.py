...
def init_database():
    ...
    try:
        logger.info(
            "🔄 PostgreSQL initialization started"
        )
        conn = get_db()
        cur = conn.cursor()
        ...
        cur.execute(
        """
        CREATE OR REPLACE VIEW passports AS
        SELECT *
        FROM team_passports;
        """
        )
        logger.info(
            "✅ passports compatibility view ready"
        )
        ...
        cur.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_passports_team
