...
async def handle_predict(message: types.Message, core, journal):
    ...
    if len(parts) < 2:
        await message.answer("❌ Напиши две команды: Зенит Спартак")
        return
    home, away = parts[0], parts[1]
    league = parts[2].upper() if len(parts) >= 3 and parts[2].upper() in ["EPL", "RPL", "UCL"] else "RPL"
    await message.answer(f"⏳ Анализирую {home} — {away} ({league})...")
    try:
        ...
        factors = explain_prediction(home_pass, away_pass, result["xg"]["home"], result["xg"]["away"], league)
