...
def get_db_counts():
    ...
    return result
# ============================================================
# PREDICTION (ИСПРАВЛЕНО)
# ============================================================
def calculate_prediction(match):
    """Рассчитывает прогноз для матча через существующий predict()"""
    ...
    try:
        ...
        # Если в результате есть поле "error" или "status":"error" – считаем ошибкой
...
def render_prediction(match, result):
    ...
    with p1:
        st.caption("П1")
        st.write(f"**{pct(result.get('probability', {}).get('home', 0))}**")
    with px:
        st.caption("X")
        st.write(f"**{pct(result.get('probability', {}).get('draw', 0))}**")
    with p2:
        st.caption("П2")
        st.write(f"**{pct(result.get('probability', {}).get('away', 0))}**")
...
def render_match_card(match, index):
    ...
    with st.container(border=True):
        ...
        with col2:
            if result and not result.get("error") and result.get("status") != "error":
                st.success("FAJ ✓")
            else:
                st.caption(
                    "Прогноз ожидает"
        ...
        with col2:
            if not result or result.get("error") or result.get("status") == "error":
                if st.button(
                    "🔮 Рассчитать",
